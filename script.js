document.addEventListener("DOMContentLoaded", () => {
		...
		let currentGameState = {
			...
			// ADD THIS: Stores the ID of the locked-in vamp for non-Vigilantes
			lastActionVampId: null,
			// Keep this for Swift Justice later
			// ... rest of state
		};
		...
		const AP_COST = {
			...
			CONTRACT_PAYOFF: 3,
			VENGEANCE_IS_MINE: 0,
			// Though cost is 0, still an action choice
			// --- END ADD ---
		};
		...
		function showScreen(screenId) {
			...
			if (targetScreenElement) {
				...
				if (currentActiveScreen && currentActiveScreen.id !== screenId) {
					...
					console.log(`Stored last screen: $ {
lastActiveScreenId
}`);
				}
...
screenId
				...
				if (currentActiveScreen) {
					lastActiveScreenId = currentActiveScreen.id;
					console.log(`Stored last screen before popup: $ {
lastActiveScreenId
}`);
				} else {
					...
					console.warn(`No active main screen found when showing popup $ {
screenId
}.Storing fallback: $ {
lastActiveScreenId
}`);
...
ability.description
		...
		function clearHighlights() {
			document.querySelectorAll(".grid - square.valid - target,
				.grid - square.invalid - target ").forEach((el) => el.classList.remove("
				valid - target ", "
				invalid - target "));
			}
			...
			function renderBoard(gameState) {
				...
				gameState.board.vampires?.forEach((vamp) => {
					const targetSquare = gameBoard.querySelector(`[data-coord="$ {
vamp.coord
}
"]`);
					if (targetSquare) {
...
"]`);
			...
			function updatePlayerInfoPanel(player, turn, currentAP, resources) {
				...
				// Silver Bullet Button (Gray Theme)
				if (btnSilverBullet) {
				...
				// Dispel Button (Conditional Visibility - Gray Theme)
				const canAffordDispel = currentAP >= AP_COST.DISPEL;
				// Visibility Condition: Selected, On GD, Can Afford
				const canDispel = isVampSelected && hazardOnVampSquare?.type === "Grave Dust";
				if (btnDispel) {
					// *** UPDATED: Set display based on canDispel ***
					btnDispel.style.display = canDispel ? "inline - block " : "none ";
					// If visible, disable based on AP / Control
					if (canDispel) {
				...
				// Bite Fuse Button (Conditional Visibility - Gray Theme)
				const canAffordBite = currentAP >= AP_COST.BITE_FUSE;
				// Visibility Condition: Selected, On Dynamite
				const canBite = isVampSelected && hazardOnVampSquare?.type === "Dynamite";
				if (btnBiteFuse) {
					// *** UPDATED: Set display based on canBite ***
				...
				// --- Class-Specific Ability Buttons (Gray Theme) ---
				// Note: These also inherit the gray theme via #action-bar button rule

				// Rampage Button (Outlaw Only)
				if (btnRampage) {
					const isSelectedOutlaw = selectedVampClass === 'Outlaw';
					btnRampage.style.display = isSelectedOutlaw ? 'inline-block' : 'none'; // Show/Hide
					if (isSelectedOutlaw) {
				...
				// --- Movement Buttons Visibility & State (Gray Theme) ---
				if (movementBar) {
					if (!isVampSelected) {
			...
			function updateUI() {
				...
				function executeMove(vampire, targetCoord) {
					...
					if (currentPlayerClass !== "Vigilante" && !currentGameState.lockedInVampireIdThisTurn) {
						...
						addToLog(`Locked into controlling $ {
vampire.id
}
for the rest of the turn.`);
						console.log(`Non - Vigilante Lock - in:Set to $ {
vampire.id
}`);
					}
...
CURSED by Grave Dust! `);
						} else {
							console.error("executeMove: Could not find vampire in state to apply curse!");
							addToLog(`ERROR: Failed to apply Curse from Grave Dust to $ {
vampire.id
}`);
						}
					}
...
AP)`);
				...
				* @param {string | null} [overrideFacing=null] - Optional facing direction to use instead of vampire's current facing(
				...
				console.log(`executeShoot: Determined cost = $ {
				cost
				} (apCostOverride = $ {
				apCostOverride
				},
				...
				addToLog(`Not enough AP to Shoot(Need $ {
				cost
				},
				Have $ {
				currentGameState.currentAP
				...
				$ {
				apCostOverride !== null ? "(Part of Ability)": ""
				}`);
				...
				addToLog(`Shot passes through Tombstone at $ {
				currentCoord
				} (Sharpshooter).`);
				console.log(`BH Sharpshooter ignores Tombstone at $ {
				currentCoord
				}.Shot continues.`);
				...
				hitMessage = `Silver Bullet shattered Tombstone at $ {
				currentCoord
				},
				but $ {
				findVampireById(targetPiece.coord) ? .id || "vampire"
				}
				was protected ! (SB Wasted)`;
				addToLog("Tombstone destroyed by blocked Silver Bullet.");
				} else {
				hitMessage = `Shot DESTROYED Tombstone at $ {
				currentCoord
				} ! `;
				}
				...
				hitMessage = `Shot hit Dynamite at $ {
				initialExplosionCoord
				}.It EXPLODES,
				starting potential chain reaction ! `;
				shotResolved = true;
				console.log(`Dynamite hit by shot at $ {
				initialExplosionCoord
				}.Initiating explosion processing.`);
				addToLog(`Shot triggers Dynamite at $ {
				initialExplosionCoord
				} ! `);
				...
				addToLog(`Shot passes through Grave Dust at $ {
				currentCoord
				}.`);
				...
				hitMessage = `Silver Bullet HIT & ELIMINATED enemy $ {
				eliminatedVampId
				}
				at $ {
				currentCoord
				...
				hitMessage = `Shot HIT enemy $ {
				targetPiece.id
				}
				at $ {
				currentCoord
				}.Target is CURSED(Marked Man) ! `;
				addToLog(`Marked Man: $ {
				targetPiece.id
				}
				is now CURSED ! `);
				...
				hitMessage = `Shot hit already cursed enemy $ {
				targetPiece.id
				}
				at $ {
				currentCoord
...
/**
* Executes the Dispel action: Removes Grave Dust from the vampire's current square. * @param {
object
}
vampire - The vampire object performing the action(from findVampireById). * @returns {
boolean
} - True
...
/ / ---Add Lock - inCheck Block Here---const currentPlayerClass = currentGameState.players[currentGameState.currentPlayerIndex].class;
if (currentPlayerClass !== "Vigilante" && !currentGameState.lockedInVampireIdThisTurn) {
currentGameState.lockedInVampireIdThisTurn = vampire.id;
addToLog(`Locked into controlling $ {
vampire.id
...
if (hazardIndex === -1) {
addToLog(`Cannot Dispel: No Grave Dust found at $ {
vampire.coord
}.`);
...
Dispelled Grave Dust at < span class = "math-inline" > \ {
vampire\.coord\
}\.\ ( < /span>{currentGameState.currentAP} AP left)`);

/ / 7.Update the display renderBoard(currentGameState); updateUI(); // Updates AP and potentially button states if needed
...
vampire.id
	...
	if (hazardIndex === -1) {
		addToLog(`Cannot Bite the Fuse: No Dynamite found at $ {
vampire.coord
}.`);
		return false; // Action fails, no AP cost
	}
	...
	// --- Action is Valid: Proceed ---
	console.log(`Executing Bite Fuse
for $ {
vampire.id
}
at $ {
...
Bit the Fuse at < span class = "math-inline" > \ {
vampInState\.coord\
},
removing Dynamite and becoming CURSED\ ! \ ( < /span>{currentGameState.currentAP} AP left)`);
	} else {
	...
	/ /
	7. Update the display renderBoard(currentGameState); // Show curse border, remove hazard
...
function nextTurn() {
	...
	const potentialSwiftJusticeVampId = currentGameState.lastActionVampId; // Get ID of last acting vamp
	let swiftJusticeTriggered = false; // Flag to check if we showed the custom popup
	...
	if (endingPlayer ? .class === "Sheriff" && !endingPlayer.eliminated && potentialSwiftJusticeVampId) {
		...
		if (lastVamp && lastVamp.player === endingPlayerIndex && !lastVamp.cursed) {
			...
			const sjPopup = popups.swiftJustice; // Get popup reference
			const sjMessage = document.getElementById('swift-justice-message'); // Get message element reference
			if (sjPopup && sjMessage) {
				// Update the message text
				sjMessage.textContent = `The Sheriffs are duty bound.Execute Swift Justice
for $ {
lastVamp.id
} ? (Move 1 free)`;
				// Show the popup
				sjPopup.style.display = 'flex';
				swiftJusticeTriggered = true; // Mark that we paused and showed the popup
				// The event listeners for the Yes/No buttons will handle the next step
			} else {
				console.error("Swift Justice popup or message element not found!");
				addToLog("Error showing Swift Justice prompt. Proceeding without.");
				// swiftJusticeTriggered remains false, so turn advances below
			}
			// --- End Custom Popup Logic ---
		} else {
			// Log why SJ wasn't offered (e.g., last vamp cursed, or ID mismatch) only if a potential vamp existed
			if (lastVamp && lastVamp.cursed) {
				addToLog("Swift Justice cannot be performed by a cursed Sheriff.");
			} else if (potentialSwiftJusticeVampId) {
				// Log other potential issues if needed for debugging
				console.warn(`Swift Justice check failed: Last Vamp ID $ {
...
lastVamp ? .cursed
			...
			// Proceed normally below (swiftJusticeTriggered is false)
		}
	} // else: Not a Sheriff or other conditions failed, proceed normally below.
	// 5. Proceed to next player ONLY IF the Swift Justice popup was NOT shown
	if (!swiftJusticeTriggered) {
		console.log("No Swift Justice trigger or prompt error, proceeding to next turn immediately.");
		saveStateToHistory(); // Save the final state of the turn that just ended
		proceedToNextPlayerTurn(); // Transition to the next player's turn
	}
	// If swiftJusticeTriggered is true, this function ends here, waiting for Yes/No button click.
}
...
vampire.id
	...
	if (!isValidSwiftJusticeTarget(targetCoord, vampire.id)) {
		addToLog(`Invalid Swift Justice move target: $ {
targetCoord || "Off-board"
}.Choose another direction.`);
		return; // Keep waiting for valid input, DO NOT proceed to next turn yet.
	}

	// --- Move is Valid ---
	console.log(`Executing Swift Justice move
for $ {
vampire.id
}
to $ {
...
uses RAMPAGE ! Firing Left & Right... ($ {
currentGameState.currentAP
}
AP left)`);
...
function isValidSwiftJusticeTarget(targetCoord, movingVampId) {
	...
	if (pieceAtTarget) {
		...
		if (pieceAtTarget.type === "vampire") {
			...
			console.log(`Swift Justice Validation Fail: Blocked by Vampire $ {
pieceAtTarget.piece.id
}`);
			return false;
		}
...
CURSED by Grave Dust after Swift Justice ! `);
	}
...
CURED by Bloodbath after Swift Justice ! `);
	}
...
function proceedToNextPlayerTurn() {
	...
	while (currentGameState.players[nextPlayerIndex] ? .eliminated && loopCheck < numberOfPlayers) {
		console.log(`nextTurn / proceed: Skipping P$ {
nextPlayerIndex
}
because eliminated status is: $ {
...
's turn (${currentPlayer.class}). AP: ${currentGameState.currentAP} ---`);
	} else {
...
function handleVampireSelection(event) {
	...
	if (clickedVampireElement) {
		...
		if (ownerIndex === currentGameState.currentPlayerIndex) {
			...
			// Clicked opponent's vampire addToLog("Cannot select opponent's vampire.");
			if (currentGameState.selectedVampireId) {
				currentGameState.selectedVampireId = null;
				if (movementBar) movementBar.classList.add("hidden");
				renderBoard(currentGameState);
...
function populateHazardPicker() {
	...
	const createBtn = (type, icon, cost) => {
		...
		btn.innerHTML = ` < span class = "hazard-icon" > $ {
icon
} < /span> ${type} <span class="hazard-cost">(${cost} AP)</span > `;
...
hazardType
	...
	popups.hazardPicker.style.display = "none";
	/* Use reference from popups object */
...
function highlightThrowTargets() {
	...
	for (let dist = 1; dist <= 3; dist++) {
		...
		if (!pathClear) {
			console.log(`Path already blocked before reaching distance $ {
dist
}`);
			break;
		}
...
function processExplosionQueue(explosionQueue, processedExplosions) {
	...
	while (explosionQueue.length > 0) {
		...
		if (processedExplosions.has(coordToExplode)) {
			console.log(`Skipping already processed explosion at $ {
coordToExplode
}`);
			continue;
		}
		...
		if (!dynamitePieceInfo || dynamitePieceInfo.type !== "hazard" || dynamitePieceInfo.piece.type !== "Dynamite") {
			console.log(`No Dynamite found at $ {
coordToExplode
} (already destroyed ? ), skipping explosion.`);
			continue; // Skip if no dynamite is actually here anymore
		}

		// 4. It exists! Log and Remove the Dynamite
		console.log(`Exploding Dynamite at $ {
coordToExplode
}`);
		...
		explosionAreaCoords.forEach((coordInBlast) => {
			...
			if (pieceInBlast) {
				...
				if (affectedType === "bloodwell") {
					console.log(`Explosion destroyed Bloodwell $ {
affectedPiece.id
}
at $ {
coordInBlast
}`);
					addToLog(`Explosion destroyed Bloodwell $ {
affectedPiece.id
}
at $ {
coordInBlast
				...
				// Check if it's a hazard AND it's NOT a dynamite that's already exploded in this chain
					...
					if (affectedPiece.type === "Dynamite") {
						...
						if (!processedExplosions.has(affectedPiece.coord)) {
							...
							console.log(`Explosion triggers another Dynamite at $ {
  affectedPiece.coord
}.Adding to queue.`);
							addToLog(`Explosion triggers nearby Dynamite at $ {
  affectedPiece.coord
} ! `);
						...
						console.log(`Explosion destroyed Hazard($ {
affectedPiece.type
}) at $ {
coordInBlast
}`);
						addToLog(`Explosion destroyed Hazard($ {
affectedPiece.type
}) at $ {
coordInBlast
} ! `);
					...
					console.log(`Explosion hit Vampire $ {
affectedPiece.id
}
at $ {
coordInBlast
}. (Effect TBD)`);
					addToLog(`Explosion hit Vampire $ {
affectedPiece.id
}
at $ {
coordInBlast
...
currentGameState\.players\ [playerIndex\] ? \.name\
}\)\: Vamps\ = </span>{remainingVamps}, BWs=${remainingBloodwells}`);

	const isEliminated = remainingVamps === 0 || remainingBloodwells === 0;

	if (isEliminated) {
		console.log(`Player <span class="math-inline">\{playerIndex\} \(</span > {
currentGameState.players[playerIndex].name
}) elimination condition met.`);
	}
...
function updateEliminationState(playerIndex) {
	...
	if (!player) {
		...
		console.error(`updateEliminationState: Invalid playerIndex $ {
playerIndex
},
cannot find player.`);
		return false;
	...
	console.log(`Updating elimination state
for P$ {
playerIndex
} ($ {
playerName
}).Setting eliminated = true.`);
...
function showEliminationPopup(playerIndex) {
	...
	if (!player) {
		console.error(`Cannot show elimination popup: Invalid playerIndex $ {
playerIndex
}
or player data missing.`);
		return;
	}
...
playerClass
	...
	if (elimPopup && elimMsg) {
		console.log(`Attempting to set popup text
for P$ {
playerIndex
}
with Name = '${playerName}'and Class = '${playerClass}'`);
...
function initializeGame() {
	console.log("Entering initializeGame. playerData:", JSON.stringify(playerData));
	console.log("Initializing game...");

	gameHistory = []; // Reset history

	const layouts = LAYOUT_DATA[numberOfPlayers]; // Get layouts for this player count
	if (!layouts?.length) {
	...
	// Select a random layout for the current player count
	...
	currentGameState = {
		...
		board: {
			// Deep copy layout vampires and add initial state properties
			vampires: JSON.parse(
				JSON.stringify(
					layout.vampires.map((v) => ({
						...v,
						cursed: false,
						movesThisTurn: 0, // Initialize moves counter
					}))
				)
			),
			// Deep copy layout bloodwells
			bloodwells: JSON.parse(JSON.stringify(layout.bloodwells)),
			// Deep copy layout hazards
			hazards: JSON.parse(JSON.stringify(layout.hazards)),
		},
		// Calculate initial hazard pool based on layout
		hazardPool: {
			...
			Dynamite: 3, // Standard starting Dynamite count
		},
		...
		lockedInVampireIdThisTurn: null, // Reset lock-in state
		lastActionVampId: null, // Reset last action tracker
		actionState: { // Reset any pending action state
			pendingAction: null,
			selectedHazardType: null
		},
		eliminatedVampires: [] // Reset list of eliminated vamps
		// *** NOTE: Duplicate actionState key removed from here ***
	};

	// --- Set Initial AP ---
	const pIdx = currentGameState.currentPlayerIndex;
	if (currentGameState.turn === 1) { // Should always be true here
		if (numberOfPlayers === 4) currentGameState.currentAP = [4, 5, 6, 8][pIdx];
		else if (numberOfPlayers === 3) currentGameState.currentAP = 6;
		else if (numberOfPlayers === 2) currentGameState.currentAP = 5;
		else currentGameState.currentAP = 5; // Default for 1 player or error?
	} else {
		// Fallback for subsequent turns (though init should only be turn 1)
		currentGameState.currentAP = 5;
	}
	console.log(`*** Calculated Initial AP: ${currentGameState.currentAP} (for player index ${pIdx}, num players ${numberOfPlayers}) ***`);

	// --- Setup Board and UI ---
	generateGrid(); // Create grid elements
	renderBoard(currentGameState); // Place initial pieces

	// Call updateUI to set initial status bar, info panel, button states etc.
	updateUI(); // This now handles the status bar color/text/AP and calls updatePlayerInfoPanel

	// --- Final Setup ---
	logList.innerHTML = `<li>Game Started: ${layoutName}</li>`; // Clear log and add start message
	gameLog.scrollTop = 0; // Scroll log to top
	if (btnUndo) btnUndo.disabled = true; // Ensure Undo is disabled initially

	if (movementBar) movementBar.classList.add("hidden"); // Ensure movement bar is hidden

	// Re-attach main gameplay listeners (remove first to prevent duplicates if init is called again)
	gameBoard.removeEventListener("click", handleBoardClick);
	gameBoard.addEventListener("click", handleBoardClick);
	if (btnUndo) btnUndo.removeEventListener("click", undoLastAction);
	...
	// Add initial turn message to log
	const player = currentGameState.players[pIdx]; // Get player again for logging
	if (player) {
...
direction
}.Swift Justice Pending ? $ {
isSwiftJusticeMovePending
}`); // Debug log
	if (isSwiftJusticeMovePending) {
...
if (btnSwiftJusticeYes) {
	btnSwiftJusticeYes.addEventListener('click', () => {
		...
		// Get player index and vamp ID from the current state at the time the prompt was shown
		swiftJusticePlayerIndex = currentGameState.currentPlayerIndex;
		swiftJusticeVampId = currentGameState.lastActionVampId;

		// Safety check
		if (swiftJusticePlayerIndex === -1 || !swiftJusticeVampId) {
			console.error("Swift Justice Yes Error: Could not retrieve player index or vamp ID.");
			// Attempt to recover by simply ending the pending state and proceeding
			isSwiftJusticeMovePending = false;
			saveStateToHistory();
			proceedToNextPlayerTurn();
			return;
...
will execute Swift Justice.Select move direction(N, E, S, W).`);
		updateUI(); // Update UI to enable D-Pad and reflect pending state if needed
	});
} else {
	console.warn("Swift Justice YES button not found");
}
...
if (btnSwiftJusticeNo) {
	btnSwiftJusticeNo.addEventListener('click', () => {
		...
		saveStateToHistory(); // Save the state before advancing turn
		proceedToNextPlayerTurn(); // Advance turn
	});
} else {
	console.warn("Swift Justice NO button not found");
}
