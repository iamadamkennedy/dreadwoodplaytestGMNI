...
function highlightThrowTargets() {
    ...
    if (!currentGameState || !currentGameState.actionState) return; // Guard clause

    const startCoord = selectedVamp.coord;
    const hazardType = currentGameState.actionState.selectedHazardType;
    if (!hazardType) return; // Ensure hazard type is set
    ...
    document.querySelectorAll('.grid-square').forEach(square => {
        ...
        if (distance > 0 && distance <= 3) {
            // --- TODO: Add path & target occupancy validation ---
            const pieceAtTarget = findPieceAtCoord(targetCoord);
            if (!pieceAtTarget || (hazardType === 'Grave Dust' && pieceAtTarget.type === 'vampire')) {
                 // Placeholder: Assume path is clear for now
                 isValid = true;
             }
             // --- End Placeholder ---
        }
        if (isValid) {
            ...
            // square.classList.add('invalid-target'); // Optionally mark invalid
        }
    });
}
// --- End Functions for Throw Action ---
